...
const Sklar2Miercoles = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("sklarMiercoles").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("sklarMiercoles").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`sklarMiercoles/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("sklarMiercoles").doc(id).update({
        nombre: nuevoNombre,
...
export default Sklar2Miercoles;
