...
const Sklar2Jueves = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("SklarJueves").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("sklarJueves").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`sklarJueves/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("sklarJueves").doc(id).update({
        nombre: nuevoNombre,
