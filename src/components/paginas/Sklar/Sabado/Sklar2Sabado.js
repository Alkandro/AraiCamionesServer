...
const Sklar2Sabado = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("sklarSabado").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("sklarSabado").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`sklarSabado/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("sklarSabado").doc(id).update({
        nombre: nuevoNombre,
