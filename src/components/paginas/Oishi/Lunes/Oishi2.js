...
const Oishi2 = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("oishi").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("oishi").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`oishi/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("oishi").doc(id).update({
        nombre: nuevoNombre,
