...
const Oishi2Viernes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("oishiViernes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("oishiViernes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`oishiViernes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("oishiViernes").doc(id).update({
        nombre: nuevoNombre,
