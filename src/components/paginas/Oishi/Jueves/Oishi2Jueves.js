...
const Oishi2Jueves = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("oishiJueves").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("oishiJueves").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`oishiJueves/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("oishiJueves").doc(id).update({
        nombre: nuevoNombre,
