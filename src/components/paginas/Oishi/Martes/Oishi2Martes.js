...
const Oishi2Martes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("oishiMartes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("oishiMartes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`oishiMartes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("oishiMartes").doc(id).update({
        nombre: nuevoNombre,
