...
const Oishi2Miercoles = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("oishiMiercoles").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("oishiMiercoles").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`oishiMiercoles/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("oishiMiercoles").doc(id).update({
        nombre: nuevoNombre,
...
export default Oishi2Miercoles;
