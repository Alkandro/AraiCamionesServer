...
const Oishi2Domingo = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("oishiDomingo").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("oishiDomingo").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`oishiDomingo/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("oishiDomingo").doc(id).update({
        nombre: nuevoNombre,
