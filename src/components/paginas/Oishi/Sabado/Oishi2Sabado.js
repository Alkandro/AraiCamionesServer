...
const Oishi2Sabado = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("oishiSabado").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("oishiSabado").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`oishiSabado/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("oishiSabado").doc(id).update({
        nombre: nuevoNombre,
