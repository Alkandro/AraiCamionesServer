...
const Tomaoka2Miercoles = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("tomaokaMiercoles").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("tomaokaMiercoles").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`tomaokaMiercoles/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("tomaokaMiercoles").doc(id).update({
        nombre: nuevoNombre,
...
export default Tomaoka2Miercoles;
