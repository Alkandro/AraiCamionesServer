...
const Tomaoka2Martes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("tomaokaMartes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("tomaokaMartes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`tomaokaMartes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("tomaokaMartes").doc(id).update({
        nombre: nuevoNombre,
