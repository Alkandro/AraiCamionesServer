...
const Tomaoka2Viernes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("tomaokaViernes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("tomaokaViernes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`tomaokaViernes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("tomaokaViernes").doc(id).update({
        nombre: nuevoNombre,
...
export default Tomaoka2Viernes;
