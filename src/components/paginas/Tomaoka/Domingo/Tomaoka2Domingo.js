...
const Tomaoka2Domingo = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("tomaokaDomingo").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("tomaokaDomingo").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`tomaokaDomingo/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("tomaokaDomingo").doc(id).update({
        nombre: nuevoNombre,
...
export default Tomaoka2Domingo;
