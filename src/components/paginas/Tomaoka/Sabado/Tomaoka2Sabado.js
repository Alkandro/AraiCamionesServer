...
const Tomaoka2Sabado = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("tomaokaSabado").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("tomaokaSabado").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`tomaokaSabado/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("tomaokaSabado").doc(id).update({
        nombre: nuevoNombre,
