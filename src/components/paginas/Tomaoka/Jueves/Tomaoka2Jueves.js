...
const Tomaoka2Jueves = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("tomaokaJueves").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("tomaokaJueves").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`tomaokaJueves/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("tomaokaJueves").doc(id).update({
        nombre: nuevoNombre,
