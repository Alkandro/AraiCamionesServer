...
const Hoshino2Sabado = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("hoshinoSabado").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("hoshinoSabado").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`hoshinoSabado/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("hoshinoSabado").doc(id).update({
        nombre: nuevoNombre,
