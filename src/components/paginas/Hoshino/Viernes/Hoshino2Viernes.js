...
const Hoshino2Viernes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("hoshinoViernes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("hoshinoViernes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`hoshinoViernes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("hoshinoViernes").doc(id).update({
        nombre: nuevoNombre,
...
export default Hoshino2Viernes;
