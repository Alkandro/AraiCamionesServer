...
const Hoshino2Miercoles = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("hoshinoMiercoles").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("hoshinoMiercoles").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`hoshinoMiercoles/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("hoshinoMiercoles").doc(id).update({
        nombre: nuevoNombre,
...
export default Hoshino2Miercoles;
