...
const Matsushima2Sabado = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("matsushimaSabado").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("matsushimaSabado").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`matsushimaSabado/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("matsushimaSabado").doc(id).update({
        nombre: nuevoNombre,
...
export default Matsushima2Sabado;
