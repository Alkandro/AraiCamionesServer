...
const Matsushima2Viernes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("matsushimaViernes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("matsushimaViernes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`matsushimaViernes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("matsushimaViernes").doc(id).update({
        nombre: nuevoNombre,
...
export default Matsushima2Viernes;
