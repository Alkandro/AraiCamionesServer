...
const Matsushima2Domingo = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("matsushimaDomingo").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("matsushimaDomingo").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`matsushimaDomingo/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("matsushimaDomingo").doc(id).update({
        nombre: nuevoNombre,
...
export default Matsushima2Domingo;
