...
const Matsushima2Miercoles = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("matsushimaMiercoles").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("matsushimaMiercoles").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`matsushimaMiercoles/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("matsushimaMiercoles").doc(id).update({
        nombre: nuevoNombre,
...
export default Matsushima2Miercoles;
