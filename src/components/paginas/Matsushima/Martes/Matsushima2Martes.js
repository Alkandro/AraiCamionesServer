...
const Matsushima2Martes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("matsushimaMartes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("matsushimaMartes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`matsushimaMartes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("matsushimaMartes").doc(id).update({
        nombre: nuevoNombre,
...
export default Matsushima2Martes;
