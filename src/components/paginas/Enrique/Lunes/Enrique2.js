...
const Enrique2 = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("enrique").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("enrique").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`enrique/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("enrique").doc(id).update({
        nombre: nuevoNombre,
