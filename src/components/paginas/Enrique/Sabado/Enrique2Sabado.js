...
const Enrique2Sabado = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("enriqueSabado").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("enriqueSabado").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`enriqueSabado/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("enriqueSabado").doc(id).update({
        nombre: nuevoNombre,
