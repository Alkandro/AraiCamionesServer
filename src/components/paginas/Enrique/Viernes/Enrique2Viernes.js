...
const Enrique2Viernes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("enriqueViernes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("enriqueViernes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`enriqueViernes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("enriqueViernes").doc(id).update({
        nombre: nuevoNombre,
...
export default Enrique2Viernes;
