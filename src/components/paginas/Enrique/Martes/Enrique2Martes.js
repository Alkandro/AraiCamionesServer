...
const Enrique2Martes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("enriqueMartes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("enriqueMartes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`enriqueMartes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("enriqueMartes").doc(id).update({
        nombre: nuevoNombre,
