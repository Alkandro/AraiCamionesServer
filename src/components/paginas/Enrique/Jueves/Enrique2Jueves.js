...
const Enrique2Jueves = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("enriqueJueves").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("enriqueJueves").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`enriqueJueves/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("enriqueJueves").doc(id).update({
        nombre: nuevoNombre,
