...
const Enrique2Miercoles = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("enriqueMiercoles").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("enriqueMiercoles").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`enriqueMiercoles/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("enriqueMiercoles").doc(id).update({
        nombre: nuevoNombre,
...
export default Enrique2Miercoles;
