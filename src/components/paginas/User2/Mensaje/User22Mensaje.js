...
const User22Mensaje = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user2Mensaje").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user2Mensaje").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user2Mensaje/${nuevaImagen.name}`);
        await imagenRef.put(nuevaImagen);
        nuevaUrlImagen = await imagenRef.getDownloadURL();
      }

      await firebase.db.collection("user2Mensaje").doc(id).update({
