...
const User22Viernes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user2Viernes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user2Viernes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user2Viernes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user2Viernes").doc(id).update({
        nombre: nuevoNombre,
