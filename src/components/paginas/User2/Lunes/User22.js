...
const User22 = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user2").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user2").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user2/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user2").doc(id).update({
        nombre: nuevoNombre,
