...
const User22Domingo = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user2Domingo").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user2Domingo").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user2Domingo/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user2Domingo").doc(id).update({
        nombre: nuevoNombre,
