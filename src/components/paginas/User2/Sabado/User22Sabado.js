...
const User22Sabado = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user2Sabado").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user2Sabado").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user2Sabado/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user2Sabado").doc(id).update({
        nombre: nuevoNombre,
