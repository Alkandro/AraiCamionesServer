...
const User12Jueves = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user1Jueves").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user1Jueves").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user1Jueves/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user1Jueves").doc(id).update({
        nombre: nuevoNombre,
