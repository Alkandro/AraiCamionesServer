...
const User12Viernes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user1Viernes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user1Viernes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user1Viernes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user1Viernes").doc(id).update({
        nombre: nuevoNombre,
