...
const User12Miercoles = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user1Miercoles").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user1Miercoles").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user1Miercoles/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user1Miercoles").doc(id).update({
        nombre: nuevoNombre,
...
export default User12Miercoles;
