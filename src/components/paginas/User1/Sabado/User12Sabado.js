...
const User12Sabado = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user1Sabado").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user1Sabado").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user1Sabado/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user1Sabado").doc(id).update({
        nombre: nuevoNombre,
