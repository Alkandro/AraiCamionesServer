...
const User12Martes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user1Martes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user1Martes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user1Martes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user1Martes").doc(id).update({
        nombre: nuevoNombre,
