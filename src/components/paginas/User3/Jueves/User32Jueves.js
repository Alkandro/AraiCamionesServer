...
const User32Jueves = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user3Jueves").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user3Jueves").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user3Jueves/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user3Jueves").doc(id).update({
        nombre: nuevoNombre,
