...
const User32Martes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user3Martes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user3Martes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user3Martes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user3Martes").doc(id).update({
        nombre: nuevoNombre,
