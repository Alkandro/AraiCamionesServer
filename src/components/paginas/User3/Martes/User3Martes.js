...
import User32Martes from "./User32Martes";
...
const User3Martes = () => {
  ...
  useEffect(() => {
    const obtenerPlatillos = () => {
      firebase.db.collection("user3Martes").onSnapshot(manejarSnapshot);
    };
    obtenerPlatillos();
  }, [firebase]);
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6">
      ...
      {platillos.map((platillo) => (
        <User32Martes key={platillo.id} platillo={platillo} />
      ))}
    </div>
  );
};

export default User3Martes;
