...
const User32Sabado = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user3Sabado").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user3Sabado").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user3Sabado/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user3Sabado").doc(id).update({
        nombre: nuevoNombre,
