...
const User32Domingo = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user3Domingo").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user3Domingo").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user3Domingo/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user3Domingo").doc(id).update({
        nombre: nuevoNombre,
