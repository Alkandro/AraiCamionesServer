...
const User32Miercoles = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("user3Miercoles").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("user3Miercoles").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`user3Miercoles/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("user3Miercoles").doc(id).update({
        nombre: nuevoNombre,
...
export default User32Miercoles;
