...
const Yamakado2Miercoles = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("yamakadoMiercoles").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("yamakadoMiercoles").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`yamakadoMiercoles/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("yamakadoMiercoles").doc(id).update({
        nombre: nuevoNombre,
...
export default Yamakado2Miercoles;
