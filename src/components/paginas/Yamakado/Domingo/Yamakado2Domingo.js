...
const Yamakado2Domingo= ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("yamakadoDomingo").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("yamakadoDomingo").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`yamakadoDomingo/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("yamakadoDomingo").doc(id).update({
        nombre: nuevoNombre,
...
export default Yamakado2Domingo;
