...
const Yamakado2Martes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("yamakadoMartes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("yamakadoMartes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`yamakadoMartes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("yamakadoMartes").doc(id).update({
        nombre: nuevoNombre,
...
export default Yamakado2Martes;
