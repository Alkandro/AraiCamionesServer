...
const Yamakado2Jueves = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("yamakadoJueves").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("yamakadoJueves").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`yamakadoJueves/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("yamakadoJueves").doc(id).update({
        nombre: nuevoNombre,
...
export default Yamakado2Jueves;
