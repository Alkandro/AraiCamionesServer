...
const Yamakado2Sabado = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("yamakadoSabado").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("yamakadoSabado").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`yamakadoSabado/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("yamakadoSabado").doc(id).update({
        nombre: nuevoNombre,
...
export default Yamakado2Sabado;
