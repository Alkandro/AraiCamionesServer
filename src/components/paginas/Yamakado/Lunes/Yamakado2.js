...
const Yamakado2 = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("yamakado").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("yamakado").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`yamakado/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("yamakado").doc(id).update({
        nombre: nuevoNombre,
