...
const Yamakado2Viernes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("yamakadoViernes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("yamakadoViernes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`yamakadoViernes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("yamakadoViernes").doc(id).update({
        nombre: nuevoNombre,
...
export default Yamakado2Viernes;
