...
const Okamoto2Miercoles = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("okamotoMiercoles").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("okamotoMiercoles").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`okamotoMiercoles/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("okamotoMiercoles").doc(id).update({
        nombre: nuevoNombre,
...
export default Okamoto2Miercoles;
