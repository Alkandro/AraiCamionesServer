...
const Okamoto2Sabado = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("okamotoSabado").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("okamotoSabado").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`okamotoSabado/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("okamotoSabado").doc(id).update({
        nombre: nuevoNombre,
