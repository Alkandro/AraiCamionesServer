...
const Okamoto2Viernes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("okamotoViernes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("okamotoViernes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`okamotoViernes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("okamotoViernes").doc(id).update({
        nombre: nuevoNombre,
...
export default Okamoto2Viernes;
