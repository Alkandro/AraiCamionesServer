...
const Okamoto2 = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("okamoto").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("okamoto").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`okamoto/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("okamoto").doc(id).update({
        nombre: nuevoNombre,
