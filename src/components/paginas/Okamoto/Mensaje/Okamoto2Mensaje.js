...
const Okamoto2Mensaje = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("okamotoMensaje").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("okamotoMensaje").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`okamotoMensaje/${nuevaImagen.name}`);
        await imagenRef.put(nuevaImagen);
        nuevaUrlImagen = await imagenRef.getDownloadURL();
      }

      await firebase.db.collection("okamotoMensaje").doc(id).update({
...
export default Okamoto2Mensaje;
