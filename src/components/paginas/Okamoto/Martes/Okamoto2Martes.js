...
const Okamoto2Martes = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("okamotoMartes").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("okamotoMartes").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`okamotoMartes/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("okamotoMartes").doc(id).update({
        nombre: nuevoNombre,
