...
const Okamoto2Jueves = ({ platillo }) => {
  ...
  const actualizarDisponibilidad = () => {
    ...
    try {
      firebase.db.collection("okamotoJueves").doc(id).update({
  ...
  const eliminarPedido = () => {
    try {
      firebase.db.collection("okamotoJueves").doc(id).delete();
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const actualizarPlatillo = async () => {
    try {
      ...
      if (nuevaImagen) {
        ...
        const imagenRef = storageRef.child(`okamotoJueves/${nuevaImagen.name}`);
      ...
      await firebase.db.collection("okamotoJueves").doc(id).update({
        nombre: nuevoNombre,
